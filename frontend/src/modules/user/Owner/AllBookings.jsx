...
const AllProperty = () => {
   ...
   const getAllProperty = async () => {
      try {
         const response = await axios.get('https://house-rent-qr83.onrender.com/api/owner/getallbookings', {
   ...
   const handleStatus = async (bookingId, propertyId, status) => {
      try {
         const res = await axios.post('https://house-rent-qr83.onrender.com/api/owner/handlebookingstatus', { bookingId, propertyId, status }, {
            headers: { 'Authorization': `Bearer ${localStorage.getItem("token")}` }
         })
         if (res.data.success) {
